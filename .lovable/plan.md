...
- A save affordance (checkmark button) makes it clear how to commit the edit.
...
2. **Save affordance**: add a small checkmark button to the right of the textarea when the transcript is dirty, in the same style as the "continue this note" bar. Blur still auto-saves, but the checkmark gives a clear target.
3. **Save path**: generalize the existing `saveBody` helper so it writes `transcript` for voice notes and `body` for typed ones, then sets status to `processing` and invokes `process-note` with `regenerate: true` exactly as it does today.
4. **Meta line**: the "exactly as you said it" caption no longer fits an edited transcript — show it only when the words are unedited, otherwise drop it.
