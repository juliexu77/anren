...
At the very end of the note — after the transcript, before "Ask about this note" — a quiet underlined italic line: *continue this note*.

- Tapping it opens a small continuation area right there: a text field with a mic button beside it.
...
- The bottom composer keeps doing exactly what it does today — a new note — so nothing about it changes or becomes ambiguous.
...
- Recorder gains an optional `continuesNoteId` argument, used by the in-note continuation control.
