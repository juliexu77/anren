...
export const NightSkyBackground = ({ children }: NightSkyBackgroundProps) => {
  return (
    <div className="relative min-h-screen">
      ...
      {/* Patchy fog — theme-aware using CSS variable */}
      <div
        className="fixed inset-0 pointer-events-none"
        style={{
          zIndex: 0,
          background: `
            radial-gradient(ellipse 60% 40% at 15% 20%, hsl(var(--fog) / 0.35) 0%, transparent 70%),
            radial-gradient(ellipse 50% 35% at 80% 15%, hsl(var(--fog) / 0.25) 0%, transparent 65%),
            radial-gradient(ellipse 70% 30% at 50% 60%, hsl(var(--fog) / 0.15) 0%, transparent 60%),
            radial-gradient(ellipse 40% 50% at 90% 75%, hsl(var(--fog) / 0.2) 0%, transparent 65%),
            radial-gradient(ellipse 55% 35% at 25% 85%, hsl(var(--fog) / 0.18) 0%, transparent 60%)
          `,
        }}
      />
