/**
 * PCM capture helpers — encode Float32 mic samples into a complete WAV file.
 * A full WAV (rather than MediaRecorder fragments) is decodable everywhere,
...
/** Concatenate PCM chunks and write a 16-bit mono WAV at `targetRate`. */
export function encodeWav(chunks: Float32Array[], sampleRate: number, targetRate = 16000): Blob {
  const samples = downsample(concat(chunks), sampleRate, targetRate);
